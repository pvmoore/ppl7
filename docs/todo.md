...
```c#
const int a       // a cannot be reassigned
int a             // a can be reassigned

immutable int a       // this in an error
const immutable int a // this is an error

// both the var and the data are const
const immutable int[3] array = [1,2,3]
const immutable int* p

// only the data is const
immutable int[3] array = [1,2,3]
immutable int* p

int[3] array2 = array // this is ok because it is a copy

// var can be reassigned and data can be modified
int[3] array = [1,2,3]
int* p

fn foo() -> int*                  // ok - pointing to mutable data
fn foo() -> immutable int*        // ok - pointing to immutable data
fn foo() -> const int*            // this is an error
fn foo() -> const immutable int*  // this is an error


a = 10 as immutable int       // this is an error
a = 10 as const int           // this is an error
a = b as immutable int*       // ok

// use var for mutable variables and mut for mutable data
immutable int[3] a      // a is reassignable, data is immutable
immutable int* a        // a is reassignable, data is immutable
int[3] a                // a is reassignable, data is mutable
int* a                  // a is reassignable, data is mutable
const int[3] a          // a is const, data is mutable
...
B[3] a                // a is reassignable, data is mutable
immutable B[3] a      // a is const,        data is immutable
...
A a
immutable A b       // error
immutable A[2] b    // b is reassignable, data is immutable


struct Z {
  const int a     // must be set on creation
}

Z[3] zzz        // contains const members. should we enforce initialisation?
```

1) Taking the address of a const variable should be allowed.
